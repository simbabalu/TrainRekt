...
describe('useTrainingScenario NEXT flow', () => {
  it('transitions decision -> transaction-inspection via NEXT using the recommendation hook', () => {
    useRecommendedTrainingMock.mockImplementation((currentExerciseId?: string | null) => {
      if (!currentExerciseId) return firstDecision;
      if (currentExerciseId === firstDecision.id) return firstInspection;
  ...
  it('transitions signature-simulation -> transaction-inspection via NEXT', () => {
    useRecommendedTrainingMock.mockImplementation((currentExerciseId?: string | null) => {
      if (!currentExerciseId) return firstSignature;
      if (currentExerciseId === firstSignature.id) return firstInspection;
      return secondInspection;
    });
  ...
  it('transitions transaction-inspection -> another eligible exercise via NEXT', () => {
    useRecommendedTrainingMock.mockImplementation((currentExerciseId?: string | null) => {
      if (!currentExerciseId) return firstInspection;
      if (currentExerciseId === firstInspection.id) return secondInspection;
    ...
    expect(controller.currentExercise.id).toBe(firstInspection.id);

    act(() => {
      controller.nextExercise();
    });
    expect(controller.currentExercise.id).toBe(secondInspection.id);
  });
  ...
  it('clears answered state when DEV exercise picker selects an exercise', () => {
    ...
    act(() => {
      controller.debugSelectExercise(firstInspection.id);
    });
